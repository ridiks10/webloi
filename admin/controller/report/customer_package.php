<?php
class ControllerReportCustomerPackage extends Controller {
	public function index() {
		$this->load->language('report/customer_package');
...
*/
		...
		$totalPackage = 0;
		if($filter_customer != ''){
			$results = $this->model_report_customer->getCustomerDoanhSo($filter_data);
...
/*
		...
		$pagination->url = $this->url->link('report/customer_package', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
*/
	...
	/*	$data['filter_ip'] = $filter_ip;
		*/
		...
		$this->response->setOutput($this->load->view('report/customer_package.tpl', $data));
	}
}