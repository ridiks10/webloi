<?php
	class ControllerSimpleBlogArticle extends Controller {
		...
		public function getForm() {
			...
			$data['button_add_short_description'] = $this->language->get('button_add_description');
		...
		private function validateForm() {
			...
			foreach ($this->request->post['article_description'] as $language_id => $value) {
				if ((strlen($value['article_title']) < 1) || (strlen($value['article_title']) > 500)) {
				...
				if (strlen($value['short_description']) < 3) {
			...
			if ((utf8_strlen($this->request->post['keyword']) < 1) || (utf8_strlen($this->request->post['keyword']) > 500)) {
