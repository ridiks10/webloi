...
class ModelReportCustomer extends Model {
	...
	public function getCustomerDoanhSo($data = array()) {
		$sql = "SELECT customer_id FROM " . DB_PREFIX . "customer";
		$implode = array();
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(lastname, ' ', firstname) LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
		...
		if($rowCustomer){
			...
			$resultPackage = $this->model_sale_customer->getTransactionsPackage($customer_id);
			if(count($resultPackage)>0){
				foreach ($resultPackage as $package) {
					$idPackage = $package['id_package'];
					$this->getChildDoanhSo($arrNode,$idPackage,$data);
				}
			}else{
				$this->getChildDoanhSo($arrNode,$customer_id*(-1),$data);
			}
		}
		return $arrNode;
	}
	
	public function getChildDoanhSo(&$arrNode,$idPackage,$data){

		$this->load->model('sale/customer');
		if($idPackage>0){
			$listChild = $this->model_sale_customer->getListChild($idPackage);
		}else{
			$listChild = $this->model_sale_customer->getListChildNotPackage($idPackage);
		}
			
			foreach ($listChild as $child) {
				if(empty($data['filter_date_start']) && empty($data['filter_date_end'])){
					array_push($arrNode , $child);
				}else if(!empty($data['filter_date_start']) && strtotime($child['date_added']) >= strtotime($data['filter_date_start']) && !empty($data['filter_date_end']) && strtotime($child['date_added']) <= strtotime($data['filter_date_end'])) {
					array_push($arrNode , $child);
				}else if(empty($data['filter_date_start']) && !empty($data['filter_date_end']) && strtotime($child['date_added']) <= strtotime($data['filter_date_end'])){
					array_push($arrNode , $child);
				}else if(!empty($data['filter_date_start']) && strtotime($child['date_added']) >= strtotime($data['filter_date_start']) && empty($data['filter_date_end'])){
					array_push($arrNode , $child);
				}
				//$this->getChildDoanhSo($arrNode,$child['id_package'],$data);
	...
	public function getCustomerPackages($data = array()) {
		$sql = "SELECT cm.*,ml.name_vn AS package_vn,CONCAT(c.lastname, ' ', c.firstname) as name_customer,c.cmnd FROM " . DB_PREFIX . "customer_ml cm LEFT JOIN " . DB_PREFIX . "customer c ON (cm.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "member_level ml ON (cm.level = ml.id)";

		$implode = array();
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(c.lastname, ' ', c.firstname) LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
		if (!empty($data['filter_date_start'])) {
			$implode[] = "DATE(cm.date_added) >= '" . $this->db->escape($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = "DATE(cm.date_added) <= '" . $this->db->escape($data['filter_date_end']) . "'";
		}
		
		if (!empty($data['filter_area'])) {
			$implode[] = "c.area = '" . $data['filter_area'] . "'";
		}
		...
		$sql .= " ORDER BY cm.date_added DESC";
	...
	public function getTotalCustomerPackages($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer_ml cm LEFT JOIN " . DB_PREFIX . "customer c ON (cm.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "member_level ml ON (cm.level = ml.id)";
	...
	public function getNameParent($id_package) {
		 if ($id_package > 0){	
			$query = $this->db->query("SELECT CONCAT(c.lastname, ' ', c.firstname,'(Gói:',ml.name_vn,' - Tiền đầu tư:',cm.money_invest,'- Số đầu tư:',cm.number_contract,' - Ngày đầu tư:',DATE_FORMAT(cm.date_added,'%d/%m/%Y'),')') AS name_package FROM " . DB_PREFIX . "customer_ml cm LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = cm.customer_id) LEFT JOIN " . DB_PREFIX . "member_level ml ON (cm.level = ml.id) WHERE cm.id_package = '" . (int)$id_package . "'");
			if(isset($query->row['name_package'])){
				return $query->row['name_package'];
			}else{
				return '';
			}
			
		}else if($id_package < 0){
			$id_package = $id_package * (-1);
			$query = $this->db->query("SELECT CONCAT(c.lastname, ' ', c.firstname,'(Gói:Chưa có gói đầu tư') AS name_package FROM ". DB_PREFIX . "customer c  WHERE c.customer_id = '" . (int)$id_package . "'");
			if(isset($query->row['name_package'])){
				return $query->row['name_package'];
			}else{
				return '';
	...
	public function getCMNDParent($id_package) {
		 if ($id_package > 0){	
			$query = $this->db->query("SELECT c.cmnd FROM " . DB_PREFIX . "customer_ml cm LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = cm.customer_id)  WHERE cm.id_package = '" . (int)$id_package . "'");
			if(isset($query->row['cmnd'])){
				return $query->row['cmnd'];
			}else{
				return '';
			}
			...
			$id_package = $id_package * (-1);
			$query = $this->db->query("SELECT c.cmnd FROM ". DB_PREFIX . "customer c  WHERE c.customer_id = '" . (int)$id_package . "'");
			if(isset($query->row['cmnd'])){
				return $query->row['cmnd'];
			}else{
				return '';
	...
	public function getCustomerProfit($data = array()) {
		$sql = "SELECT cm.*,ml.name_vn AS package_vn,CONCAT(c.lastname, ' ', c.firstname) as name_customer,c.cmnd,c.address_bank,c.account_bank FROM " . DB_PREFIX . "customer_ml cm LEFT JOIN " . DB_PREFIX . "customer c ON (cm.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "member_level ml ON (cm.level = ml.id)";
	...
	public function getProfitCustomerByPackage($id_package,$type,$data = array()) {
		$sql = "SELECT SUM(receive) AS total FROM " . DB_PREFIX . "profit ";
		
		$implode = array();
		$implode[] = "ml_package_id = '" . (int)$id_package ."'";
		$implode[] = "type_profit in (".$type.")";
		if (!empty($data['filter_date_start'])) {
			$implode[] = " `date` >= " . strtotime($this->db->escape($data['filter_date_start'])) ;
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = " `date` <= " . strtotime($this->db->escape($data['filter_date_end']));
		}
	...
	public function getProfitCustomerByType($user_id,$type,$data = array()) {
		$sql = "SELECT SUM(receive) AS total FROM " . DB_PREFIX . "profit ";
		
		$implode = array();
		$implode[] = "user_id = '" . (int)$user_id ."'";
		$implode[] = "type_profit in (".$type.")";
		if (!empty($data['filter_date_start'])) {
			$implode[] = " `date` >= " . strtotime($this->db->escape($data['filter_date_start'])) ;
		}
