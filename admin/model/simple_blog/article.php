<?php
	class ModelSimpleBlogArticle extends Model {
		...
		public function addArticle($data) {
			...
			if($data['simple_blog_author_id'] == ''){
				$rowAuthor = $this->getAuthorName($data['author_name']);
				$data['simple_blog_author_id'] = $rowAuthor[0]['simple_blog_author_id'];
			}
		...
		public function getAuthorName($author_name) {
			$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "simple_blog_author` WHERE LCASE(name) = '" . $this->db->escape(utf8_strtolower($author_name)) . "'");
			return $sql->rows;
		}
