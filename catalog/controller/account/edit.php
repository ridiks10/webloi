...
class ControllerAccountEdit extends Controller {
	...
	public function index() {
		...
		$data['entry_cmnd'] = $this->language->get('entry_cmnd');
		...
		$data['entry_p_node'] = $this->language->get('entry_p_node');
		...
		$data['entry_money_invest'] = $this->language->get('entry_money_invest');
		...
		if (!empty($customer_info) && is_file(DIR_IMAGE . $customer_info['img_profile'])) {
			$data['img_pro'] = $this->model_tool_image->resize($customer_info['img_profile'], 180, 225);
		} else {
			$data['img_pro'] = $this->model_tool_image->resize('catalog/logo.png', 180, 225);
		}
		
		if (!empty($customer_info) && $customer_info['p_node'] != 0) {
			$nameParent = $this->model_account_customer->getNameParent($customer_info['p_node']);
			$data['p_node'] = $nameParent;
		} else {
			$data['p_node'] = '';
		}
		
		if (!empty($customer_info)) {
			$monthReg = $this->model_account_customer->getMonthRegister($customer_info['customer_id']);
			$data['monthReg'] = $monthReg;
		} else {
			$data['monthReg'] = '';
		}
	...
	if (!empty($customer_info)) {
		...
		if (isset($this->request->post['date_birth'])) {
			...
			$data['date_birth'] = date('d-m-Y',strtotime($customer_info['date_birth']));
		} else {
			$data['date_birth'] = '';
		}
		...
		if (isset($this->request->post['date_added'])) {
			...
			$data['date_added'] = date('d-m-Y',strtotime($customer_info['date_added']));
		} else {
			$data['date_added'] = '';
		}
	...
	if (isset($this->request->post['address_bank'])) {
		...
		$data['hoiphi'] = $this->config->get('config_hoiphi');
