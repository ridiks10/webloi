    ...
    class ControllerSimpleBlogCategory extends Controller {
    	public function index() {
			...
			if($category_info) {
				...
				foreach($results as $result) {
					$article_title = utf8_substr(strip_tags(html_entity_decode($result['article_title'], ENT_QUOTES, 'UTF-8')), 0, 150) . '...';
					...
					if($simple_blog_category_id == 2){
						$data['articles'][] = array(
							...
							'href'				=> $this->url->link('simple_blog/category', 'simple_blog_category_id='.$simple_blog_category_id.'&simple_blog_article_id=' . $result['simple_blog_article_id'], 'SSL')
						);
					}else{
						$data['articles'][] = array(
							'simple_blog_article_id'	=> $result['simple_blog_article_id'],
				...
				$data['simple_blog_category_id'] = $simple_blog_category_id;
				$data['articles_gt'] = '';
				// Gioi thieu
				if($simple_blog_category_id == 2){
					if(isset($this->request->get['simple_blog_article_id'])){
						...
						$simple_blog_article_id = $data['articles'][0]['simple_blog_article_id'];
					}
					$data['simple_blog_article_id'] = $simple_blog_article_id;
					$row_gt = $this->model_simple_blog_article->getArticle($simple_blog_article_id);
					$data['articles_gt'] = $row_gt;
					// Danh muc dau tu
				}else if($simple_blog_category_id == 4){
					$children = $this->model_simple_blog_article->getCategories($simple_blog_category_id);
					foreach ($children as $child) {
						$children_data[] = array(
							'category_id' => $child['simple_blog_category_id'],
							'name'  => $child['name'],
							'href'  => $this->url->link('simple_blog/category', 'simple_blog_category_id=' . $simple_blog_category_id . '_' . $child['simple_blog_category_id'])
						);		
					}
					$data['cat_child'] = $children_data;
				}
